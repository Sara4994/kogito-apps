...
const DomainExplorerFilterOptions = ({
  ...
  const domainArg =
    ...
    console.log('domainArg', domainArg)
    ...
    !getSchema.loading &&
      ...
      getSchema.data.__type.inputFields
        ...
        .map((group, index) => {
          ...
          if (rootItem) {
            const rootElement = <SelectOption key={rootItem.name} value={rootItem.name} />
            !rootElementsArray.includes(rootElement) && rootElementsArray.push(rootElement)
          }
                  ...
                  groupItem.type.inputFields
                    .filter((item, _index) => {
                      if (!nullTypes.includes(item.type.name)) {
                        ...
                        tempData.push(_v);
                        childSelectionItems(tempData, groupItem.name, item.name);
                      } else {
                    ...
                    .map((item, _index) => {
                      return (
                        <SelectOption key={_index} value={item.name + groupItem.name}>
                          {item.name}
                        </SelectOption>
                      );
                    })}
  ...
  const onApplyFilter = async () => {
    ...
    if (currentArgumentScalar === 'Boolean') {
      ...
      const chip = `${parentString}-${selected}-${selectTypes}-${currentBoolean}`;
      ...
      typeParent.map(parent => parentString = parentString + '-' + parent)
      parentString = parentString.substring(1)
      const chip = `${parentString}-${selected}-${selectTypes}-${inputArray}`;
      if (!filterChips.includes(chip)) {
        setFilterChips(prev => [...prev, chip])
      }
      ...
      typeParent.map(parent => parentString = parentString + '-' + parent)
      parentString = parentString.substring(1)
      const chip = `${parentString}-${selected}-${selectTypes}-${textValue}`;
      if (!filterChips.includes(chip)) {
        setFilterChips(prev => [...prev, chip])
      }
      set(obj, n, textValue);
  ...
  return (
    ...
    >
      ...
      >
        <DataToolbarContent>
          <DataToolbarItem variant="label" id="stacked-example-resource-select">Field</DataToolbarItem>
          <DataToolbarItem>
            {!getSchema.loading && (
              <Select
          ...
          <DataToolbarItem variant="label" id="stacked-example-resource-select">Operator</DataToolbarItem>
          <DataToolbarItem>
          ...
          <DataToolbarItem variant="label" id="stacked-example-resource-select">Value</DataToolbarItem>
          ...
          {selectTypes.length > 0 && currentArgumentScalar === 'ArrayString' && (
            <>
              <DataToolbarItem>
                <InputGroup>
                  <TextInput 
