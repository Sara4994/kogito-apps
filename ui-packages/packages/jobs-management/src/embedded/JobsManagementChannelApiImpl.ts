...
export class JobsManagementChannelApiImpl implements JobsManagementChannelApi {
  ...
  jobList__applyFilter(filter: JobFilter): Promise<any> {
    return this.driver.applyFilter(filter);
  }

  jobList__bulkCancel(test: string, JobsIds: string[]): Promise<void> {
    return this.driver.bulkCancel(test, JobsIds);
  }

  jobList_cancelJob(jobId: string): Promise<void> {
    return this.driver.cancelJob(jobId);
  }
